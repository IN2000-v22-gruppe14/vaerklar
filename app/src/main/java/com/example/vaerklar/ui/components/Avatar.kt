...
fun Avatar(data: WeatherData?) {
    ...
    val converterClothing = hashMapOf<String?, Int>(
       ...
       "frakk" to R.drawable.frakk,
       "frakk2" to R.drawable.frakk2,
    ...
    Column(
        ...
        Box(Modifier.fillMaxSize()) {
            ...
            val top : List<String> = listOf("bobblejakke", "frakk", "tskjorte", "langermet", "ullgenser","kjole","genser"  )
            for (ord in top){
                if (ord in clothingString){
                    converterClothing.get(ord)?.let { painterResource(it) }?.let {
                        Image(
